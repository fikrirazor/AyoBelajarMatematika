...
func pertanyaan():
	...
	var hasil = stepify(qz.bagi(float(a),float(b)),0.001)
	...
	var randomsalah0
	while true:
		randomsalah0 = stepify(rand.randf_range(nilaiAwal,nilaiAkhir),0.001)
	...
	var randomsalah1
	while true:
		randomsalah1 = stepify(rand.randf_range(nilaiAwal,nilaiAkhir),0.001)
